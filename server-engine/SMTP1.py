...
from mail_from import parse as mail_from
from rcpt_to import parse as rcpt_to
from data_cmd import (parse as data_cmd, 
                      parse_data_terminator as terminator,
                      )
    
from exceptions import (UnrecognizedCommandException,
                        SyntaxException,
                        BadCmdSequenceException,
                        )

# Global var to hold the state of the program. The state is used to 
#   indicate which command is expected to be recieved next.
state: int = 0
# Global var to hold the sender that is parsed from the MAIL TO 
#   command.
sender: str = ""
# Global var to hold all of the recipients of a given message.
recipients: list[str]  = []
# Global var to hold the data sent in the message (the actual text).
message: str = ""
# Global flag to track if the data command has been terminated.
